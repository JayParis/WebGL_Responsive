...
function inputDown(event) {
    ...
    let screenX = isTouch ? event.changedTouches[0].clientX : event.x;
    let screenY = isTouch ? event.changedTouches[0].clientY : event.y;
    let inCanvas = screenX > canvasLeft && screenX < (canvasLeft + canvasWidth);
    
    if(inCanvas){
        inputType = isTouch ? 1 : event.which;
        event.preventDefault();
    }

    if(inputType == 1 && inCanvas)
        tapPos = [screenX, screenY];


    console.log(event);
    document.getElementById('olt-id').innerHTML += (event + "<br>");

    console.log("IN CANVAS: " + inCanvas);
}
